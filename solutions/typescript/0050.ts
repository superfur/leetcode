...
function myPow(x: number, n: number): number {
    ...
    // 处理负数指数：使用 Math.abs 安全地获取绝对值
    // 注意：当 n = -2147483648 时，直接 -n 可能溢出，使用 Math.abs 更安全
    const isNegative = n < 0;
    // 使用 Math.abs 处理，避免 -2147483648 取反溢出
    let exp = Math.abs(n);
    ...
    // 迭代计算：将指数转为二进制，逐位处理
    // 使用 Math.floor 代替位运算，避免大整数位运算的问题
    while (exp > 0) {
